...
/**
 * Expanso-branded glow effect system powered by blurred polygons
 * 
 * Adapted from BaizeAI template with Expanso Design System colors
 * 
 * Frontmatter Properties:
 * - glow: 'left' | 'right' | 'top' | 'bottom' | 'full' | 'center' | 'top-left' | 'top-right' | 'bottom-left' | 'bottom-right' | 'topmost'
 *   Distribution of the polygon points (default: 'full')
 ...
 * 
 * Example usage in slide frontmatter:
 * ---
 * glow: bottom-right
 * glowOpacity: 0.3
 * ---
...
<template>
  <div>
    <div
      ...
      aria-hidden="true"
    >
      <!-- Primary violet gradient - Expanso primary brand color -->
      <div
        class="clip bg-gradient-to-r from-[#501E99] to-[#6823CD]"
        :style="{ 'clip-path': `polygon(${poly1})`, 'opacity': opacity }"
      />
      <!-- Secondary violet gradient - lighter accent -->
      <div
        class="clip bg-gradient-to-l from-[#6823CD] to-[#9259ED]"
        :style="{ 'clip-path': `polygon(${poly2})`, 'opacity': opacity }"
      />
      <!-- Tertiary accent - violet-600 for subtle highlight -->
      <div
        class="clip bg-gradient-to-t from-[#883FFD] to-[#B087F1]"
